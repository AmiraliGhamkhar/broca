...
1. **Hero is a photograph, not the cancelled 3D-heart video.** Three
   AI-generated candidates were reviewed (client informed the asset is
   AI-generated, disclosed here as required). The selected `heart-model`
   artwork ships at `public/images/hero/hero.{webp,jpg}` with its final alt
   text; the temporary candidate copies and review page were removed once the
