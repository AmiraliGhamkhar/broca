...
- **`VideoController`** —
  ...
  - `media`: streams the placeholder MP4 from the **private disk** (`storage/app/private/videos/…`) behind the signed URL **and** re-checks published + entitlement — paywalled media never sits in the docroot where the web server would statically serve it with zero auth; `Cache-Control: private, no-store`. (Placeholder is provisioned by `broca:provision-placeholder-media`; a real VOD/CDN adapter replaces the provider.)
...
- **`PaymentController`** —
  ...
  - `callback`/`zibalCallback`: thin wrappers over the shared `handleGatewayCallback(okStatuses)` — ZarinPal accepts `Status === 'OK'`, Zibal accepts `Status ∈ {'1','2'}` (2 = "already verified at the gateway", still server-re-verified). Unknown/missing authority → safe redirect to plans (fail closed, never crash); already-paid invoice → **idempotent success redirect**; records one `PaymentTransaction` per (gateway, authority) — `UniqueConstraintViolationException` → reuses the row; an accepted status triggers **server-side, amount-bound verification** via the gateway (a rejection skips the network call); resolution then runs through **`PaymentFinalizer`** (row-locked, status-checked: only pending/initiated can be resolved; a paid invoice can never be downgraded); redirects to success/failed.
...
- **`ZibalGateway`** (implements `PaymentGateway`) — secondary gateway (client decision 2026-09-05); same contract: server-side verification bound to the invoice's amount + trackId; `startPayment` persists the authority, `verifyPayment` returns a `Receipt` or null on failure. Callback semantics: `paid=1`, `already-verified=2` — both reach server verification via `okStatuses: ['1','2']`.
- **`PaymentFinalizer`** — the **single resolution path used by the reconcile command**: verified payments heal *any* non-paid state (pending/initiated/failed/expired/cancelled); unverified callbacks can only fail pending/initiated; a paid invoice can never be downgraded; `ensureSubscription` repairs paid invoices missing their subscription row. `PaymentController::handleGatewayCallback` routes every gateway callback through it — there is no second, inlined resolution path.
...
- **`ActivityLogController`** — paginated audit log (100 rows/page, latest first) with user relation.
- **`TwoFactorController`** — enroll (TOTP secret generated locally, manual key entry — no QR dependency), challenge/verify (6-digit, session stamp valid 12h, `session()->regenerate()` after 2FA pass, throttled 10,1, **reused TOTP steps rejected** via `users.totp_last_step`), recover (single-use codes, consumed under row lock with `hash_equals`), enable (confirms code, issues 10 recovery codes `XXXXX-XXXXX` shown once), disable, regenerate recovery codes (`/two-factor/disable`, `/two-factor/recovery-codes` — both throttled 10,1).
...
| `Subscription` | invoice_id, status machine (`active/scheduled/expired/cancelled`), `activate()/schedule()/expire()/cancel()`, casts datetimes; `isActive()` treats a **NULL `ends_at` as never-expiring** (stays active while started, mirroring `User::activeSubscription()`) |
...
- **`Totp`** — RFC 6238 TOTP (SHA-1, 6 digits, 30s step, ±1 window), base32 secrets (RFC 4648, no padding), `otpauth://` URI for manual entry, constant-time `hash_equals` verification; fully local (no dependency). `verifyStep` returns the matched step and the caller persists it in `users.totp_last_step` — a replayed code from the same (or earlier) 30s step is rejected, so captured codes cannot be reused.
...
- **`broca:backup-database`** (daily 02:00) — `mysqldump` via Symfony Process with `MYSQL_PWD` env (password never in argv), `--single-transaction --routines --triggers`, streamed to disk in chunks, gzip, prune older than 14 days; `--ignore-table` excludes `sessions/cache/cache_locks/jobs/job_batches/failed_jobs` (transient state doesn't belong in a dump that travels to the Telegram bot chat); MySQL-only with clear errors; reminds the operator to copy backups off-box.

### 3.9 Telegram management bot (`TelegramBotService`)
- **Authorization**: webhook is signature-verified (`X-Telegram-Bot-Api-Secret-Token`, `hash_equals`) and **fails closed** — an enabled bot with an empty secret 403s every update, and registering a webhook without a secret is refused. Senders must be in `telegram_admins` (active) or `services.telegram.admin_ids`.
- **Interaction model**: text commands (`/course_new`, `/quiz_edit {id}`, …), Persian menu shortcuts, and inline-keyboard callback verbs (`menu|list|new|edit|manage|delete|cover|transition|free|setbrand|removebrand|stats|activity|userstatus|useradmin|userenroll|userunenroll|backup` + `confirm*` counterparts) over `telegram_chat_sessions` state machines (180 min TTL). Structured forms (`key: value`, `[content]` blocks) via `StructuredMessageParser`; every `findOrFail` path is caught (stale buttons answer with a Persian "not found" instead of a webhook 500).
- **Content surface**: full CRUD for plans, blogs, subjects, courses, videos, notes, decks/cards, quizzes/questions; publication **state machine** (`draft → in_review → published → archived`) with direct publishing refused and medical bylines (author ≠ reviewer) enforced when publishing; free-designation routed through `FreeItemDesignationService`; course/blog covers; subject visibility toggle; deletion confirmations with the subject-has-courses guard.
- **User surface**: paginated user list, manage card (status, role, subscription, counts), suspend/activate and promote/demote — both resolved inside `lockForUpdate` transactions enforcing the web panel's invariants (self-guards can't be replicated because `telegram_admins` carries no site `user_id`, so the bot enforces the stronger "at least one **active** admin must remain" rule); enrollment (`firstOrCreate` on the `(user, course)` unique key) and unenrollment (status → `cancelled`, history preserved).
- **Site surface**: brand logo/hero upload → `BrandAssets` (fixed names on the public disk, GD-regenerated WebP twin for the hero with an absence-marker fallback, SVG refused as stored-XSS, 8 MB cap); stats dashboard; last-10 admin activity rows; DB backup queued as a job, delivered to the chat.
- **Media rules**: bot-uploaded videos land on the **private disk** (`manifest_reference` keeps the bare filename; `media()` resolves inside the private videos dir); remote (URL) imports capped at 200 MB (declared `Content-Length` **and** observed size) with per-hop SSRF validation; `playback_url` for `source_mode=url` must match `BROCA_EXTERNAL_VIDEO_ORIGINS` or the save fails with actionable Persian copy.
