...
<section class="mx-auto max-w-5xl px-5 py-20 sm:px-8 lg:px-12 lg:py-28" x-data="videoPlayback()">
    ...
    <p class="mt-12 text-sm font-bold text-rausch">{{ $course->subject?->name }}</p>
    <h1 class="mt-4 max-w-4xl font-display text-4xl sm:text-6xl leading-tight">{{ $video->title }}</h1>

    @if ($video->description)
        <p class="mt-6 max-w-3xl text-lg leading-9 text-muted">{{ $video->description }}</p>
    @endif

    <div class="mt-12 rounded-[2rem] bg-ink p-6 text-white sm:p-10">
        <template x-if="manifest">
            <div>
                ...
                <div class="mt-5 flex flex-wrap items-center gap-4">
                    ...
                    <p class="text-sm text-white/65"><span x-text="progressPercent">0</span>٪ دیده‌شده</p>
                    <p x-show="completed" x-cloak class="text-sm font-bold text-white">✓ این ویدیو برای تو تکمیل شده است.</p>
                </div>
                <p class="mt-5 text-sm text-white/65">اگر پخش ویدیو شروع نشد، اتصال سرویس ویدیو هنوز در محیط نمونه فعال نشده است.</p>
            </div>
        </template>

        <template x-if="!manifest">
            <div class="flex min-h-64 flex-col items-center justify-center text-center">
                <span class="grid size-16 place-items-center rounded-full bg-rausch text-2xl" aria-hidden="true">▶</span>
                <h2 class="mt-6 text-2xl font-bold">ویدیو آمادهٔ یادگیری است</h2>
                @if ($canPlay)
                    <p class="mt-3 max-w-md leading-8 text-white/65">برای دریافت مجوز کوتاه‌مدت پخش، دکمهٔ زیر را بزن.</p>
                    <button type="button" x-on:click="loadPlayback" x-bind:disabled="loading" class="mt-6 rounded-full bg-rausch px-6 py-3 font-bold text-white disabled:cursor-wait disabled:opacity-60 focus:outline-none focus-visible:ring-2 focus-visible:ring-white">
                        <span x-show="!loading">دریافت مجوز پخش</span>
                        <span x-show="loading" x-cloak>در حال آماده‌سازی…</span>
                    </button>
                @else
                    <p class="mt-3 max-w-md leading-8 text-white/65">این ویدیو برای حساب تو باز نیست. ابتدا در دوره ثبت‌نام کن و برای محتوای ویژه اشتراک فعال داشته باش.</p>
                    <a href="{{ route('courses.show', $course) }}" class="mt-6 rounded-full bg-rausch px-6 py-3 font-bold text-white">مشاهدهٔ دوره</a>
                @endif
    ...
    <div class="mt-8 flex flex-wrap gap-3 text-sm font-bold">
        <span class="rounded-full bg-surface-soft px-4 py-2 font-bold">{{ $video->duration_seconds ? gmdate('i:s', $video->duration_seconds) : 'زمان نمونه' }}</span>
        <span class="rounded-full bg-teal px-4 py-2">تکمیل با مشاهدهٔ {{ $threshold }}٪</span>
        <span class="rounded-full bg-surface-strong px-4 py-2 text-ink font-bold">پخش با مجوز کوتاه‌مدت</span>
    </div>
</section>
