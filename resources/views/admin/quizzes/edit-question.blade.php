...
<section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="form-panel">
        <div class="flex items-center justify-between pb-4 border-b border-broca-sand">
            <div>
                <span class="text-xs font-bold text-coral">{{ $quiz->title }} ({{ $quiz->course->title ?? '' }})</span>
                <h2 class="text-2xl font-black text-ink mt-0.5">{{ $question->exists ? 'ویرایش سؤال آزمون' : 'افزودن سؤال چهارگزینه‌ای جدید' }}</h2>
            </div>
            <a href="{{ route('admin.quizzes.index', ['quiz_id' => $quiz->id]) }}" class="text-xs font-bold text-coral underline">بازگشت به سؤالات آزمون</a>
        </div>
        ...
        <form method="post" action="{{ $question->exists ? route('admin.quizzes.questions.update', $question) : route('admin.quizzes.questions.store') }}" class="mt-6 space-y-6">
            ...
            <div>
                <label for="prompt" class="block text-xs font-black text-ink">صورت سؤال (متن پرسش بالینی یا فیزیولوژی)</label>
                <textarea id="prompt" name="prompt" rows="3" required
                          ...
                          class="w-full mt-1.5 p-3 rounded-xl border border-ink/20 text-xs font-bold bg-white/70">{{ old('prompt', $question->prompt) }}</textarea>
            </div>

            <!-- Options & Correct Answer Selection -->
            <div class="p-5 rounded-2xl bg-white/60 border border-broca-sand space-y-4">
                <h3 class="text-xs font-black text-ink flex items-center justify-between">
                    <span>گزینه‌های پاسخ (گزینه صحیح را با دکمه رادیویی مشخص کنید)</span>
                    <span class="text-[11px] text-coral font-bold">* گزینه درست الزامی است</span>
                </h3>
                    ...
                    @for ($i = 0; $i < 4; $i++)
                        ...
                        <div class="flex items-center gap-3">
                            <label class="flex items-center gap-1.5 cursor-pointer shrink-0">
                                ...
                                <span class="text-xs font-black text-ink">گزینه {{ $i + 1 }} {{ $isCorrect ? '(صحیح)' : '' }}</span>
                            </label>
                            <input type="text" name="options[{{ $i }}][label]" value="{{ $optLabel }}" required
                                   placeholder="متن گزینه {{ $i + 1 }}..."
                                   class="flex-1 p-2.5 rounded-xl border border-ink/20 text-xs bg-white/80 font-medium">
                        </div>
                    @endfor
                </div>
            </div>
            ...
            <div>
                <label for="explanation" class="block text-xs font-black text-ink">پاسخ تشریحی و نکته آموزشی (نمایش به دانشجو پس از ارسال پاسخ)</label>
                <textarea id="explanation" name="explanation" rows="3"
                          placeholder="توضیح کامل در مورد دلیل درستی گزینه و رد سایر گزینه‌ها..."
                          class="w-full mt-1.5 p-3 rounded-xl border border-ink/20 text-xs font-medium bg-white/70">{{ old('explanation', $question->explanation) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="source_citation" class="block text-xs font-black text-ink">منبع بالینی / رفرنس علمی</label>
                    <input type="text" id="source_citation" name="source_citation" value="{{ old('source_citation', $question->source_citation) }}"
                           ...
                           class="w-full mt-1.5 p-3 rounded-xl border border-ink/20 text-xs font-medium bg-white/70">
                </div>

                <div>
                    <label for="sort_order" class="block text-xs font-black text-ink">ترتیب در آزمون</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="author_id" class="block text-xs font-black text-ink">طراح سؤال</label>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-broca-sand">
                ...
                <a href="{{ route('admin.quizzes.index', ['quiz_id' => $quiz->id]) }}" class="px-5 py-3 rounded-full border border-ink/20 text-xs font-bold text-broca-slate hover:bg-broca-sand">
                    انصراف
                </a>
            </div>
        </form>
