...
<section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="form-panel">
        <div class="flex items-center justify-between pb-4 border-b border-broca-sand">
            <div>
                <h2 class="text-2xl font-black text-ink">{{ $quiz->exists ? 'ویرایش آزمون' : 'ساخت آزمون جدید' }}</h2>
                <p class="text-xs text-broca-slate mt-1">مشخصات آزمون، دوره مربوطه و حدنصاب نمره قبولی</p>
            </div>
            <a href="{{ route('admin.quizzes.index') }}" class="text-xs font-bold text-coral underline">بازگشت</a>
        </div>
        ...
        <form method="post" action="{{ $quiz->exists ? route('admin.quizzes.update', $quiz) : route('admin.quizzes.store') }}" class="mt-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    <label for="title" class="block text-xs font-black text-ink">عنوان آزمون</label>
            ...
            <div>
                <label for="description" class="block text-xs font-black text-ink">توضیحات و راهنمای آزمون</label>
                <textarea id="description" name="description" rows="3"
                          placeholder="توضیح درباره مباحث مورد سنجش در این آزمون..."
                          class="w-full mt-1.5 p-3 rounded-xl border border-ink/20 text-xs font-medium bg-white/70">{{ old('description', $quiz->description) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="author_id" class="block text-xs font-black text-ink">استاد / طراح سؤالات</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label for="pass_threshold_percent" class="block text-xs font-black text-ink">حدنصاب قبولی (درصد)</label>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-broca-sand">
                ...
                <a href="{{ route('admin.quizzes.index') }}" class="px-5 py-3 rounded-full border border-ink/20 text-xs font-bold text-broca-slate hover:bg-broca-sand">
                    انصراف
                </a>
            </div>
        </form>
