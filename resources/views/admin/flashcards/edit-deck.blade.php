...
<section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="form-panel">
        <div class="flex items-center justify-between pb-4 border-b border-hairline-soft">
            <div>
                <h2 class="text-2xl font-bold text-ink">{{ $deck->exists ? 'ویرایش دسته کارت' : 'ساخت دسته کارت جدید' }}</h2>
                <p class="text-xs text-muted mt-1">دسته‌بندی کارت‌های مرور فاصله‌دار مربوط به هر دوره آموزشی</p>
            </div>
            <a href="{{ route('admin.flashcards.index') }}" class="text-xs font-bold text-rausch underline">بازگشت</a>
        </div>
        ...
        <form method="post" action="{{ $deck->exists ? route('admin.flashcards.decks.update', $deck) : route('admin.flashcards.decks.store') }}" class="mt-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    <label for="title" class="block text-xs font-bold text-ink">عنوان دسته کارت</label>
            ...
            <div>
                <label for="description" class="block text-xs font-bold text-ink">شرح دسته کارت</label>
                <textarea id="description" name="description" rows="3"
                          placeholder="توضیحات تکمیلی درباره مفاهیم موجود در این کارت‌ها..."
                          class="w-full mt-1.5 p-3 rounded-xl border border-ink/20 text-xs font-medium bg-white">{{ old('description', $deck->description) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="author_id" class="block text-xs font-bold text-ink">استاد / طراح کارت‌ها</label>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-hairline-soft">
                ...
                <a href="{{ route('admin.flashcards.index') }}" class="px-5 py-3 rounded-full border border-ink/20 text-xs font-bold text-muted hover:bg-surface-soft">
                    انصراف
                </a>
            </div>
        </form>
