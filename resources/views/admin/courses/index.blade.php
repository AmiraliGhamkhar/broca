...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-ink">دوره‌های آموزشی</h2>
            <p class="text-xs text-muted mt-1">مدیریت دوره‌های علوم پزشکی، محتوای دروس، اساتید و فرآیند انتشار</p>
        </div>
        <a href="{{ route('admin.courses.create') }}" class="rounded-full bg-ink px-5 py-2.5 text-xs font-bold text-white hover:bg-rausch transition-all shadow-sm">
            + افزودن دوره جدید
        </a>
    </div>
    ...
    <div class="surface-panel mt-6 p-4">
        <form method="get" action="{{ route('admin.courses.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            <select name="subject_id" class="rounded-xl border border-ink/20 p-2.5 text-xs bg-white">
            ...
            @if (request()->hasAny(['q', 'subject_id', 'status']))
                <a href="{{ route('admin.courses.index') }}" class="text-xs text-rausch font-bold underline">پاک کردن</a>
            @endif
        </form>
    </div>
    ...
    <div class="surface-panel mt-6 overflow-x-auto">
        <table class="w-full text-xs">
            <thead class="bg-surface-soft text-right text-muted border-b border-hairline-soft">
                <tr>
                    <th class="p-3.5 font-bold">عنوان دوره</th>
                    <th class="p-3.5 font-bold">درس‌نامه</th>
                    <th class="p-3.5 font-bold">نویسنده و بازبین</th>
                    <th class="p-3.5 font-bold text-center">محتوا</th>
                    <th class="p-3.5 font-bold text-center">دانشجویان</th>
                    <th class="p-3.5 font-bold">وضعیت انتشار</th>
            ...
            <tbody class="divide-y divide-broca-sand">
                    ...
                    <tr class="hover:bg-white/40">
                        <td class="p-3.5 font-bold text-ink">
                            <div class="flex items-center gap-2">
                                <span class="size-2 rounded-full {{ $course->status === 'published' ? 'bg-teal' : 'bg-surface-soft' }}"></span>
                                <a href="{{ route('courses.show', $course) }}" target="_blank" class="hover:text-rausch transition-colors font-bold text-sm">
                                    {{ $course->title }}
                                </a>
                            </div>
                            <span class="text-[11px] text-muted block mt-0.5">سطح: {{ $course->level ?: 'عمومی' }} · اسلاگ: {{ $course->slug }}</span>
                        </td>
                        <td class="p-3.5 font-bold text-muted">{{ $course->subject->name ?? '—' }}</td>
                        ...
                        <td class="p-3.5 text-center font-bold">
                            <span title="ویدیوها" class="inline-block px-1.5 py-0.5 rounded bg-surface-soft">{{ $course->videos_count }} ویدیو</span>
                            <span title="جزوات" class="inline-block px-1.5 py-0.5 rounded bg-surface-soft mt-0.5">{{ $course->notes_count }} جزوه</span>
                        </td>
                        ...
                        <td class="p-3.5">
                            <div class="flex items-center gap-2">
                                <span class="rounded-full px-2.5 py-1 text-[11px] font-bold {{ $course->status === 'published' ? 'bg-teal/10 text-teal' : ($course->status === 'in_review' ? 'bg-surface-soft text-ink' : 'bg-surface-soft text-muted') }}">
                                ...
                                <form method="post" action="{{ route('admin.publication.update', ['type' => 'courses', 'id' => $course->id]) }}">
                                    ...
                                    @if ($course->status === 'draft')
                                        ...
                                        <button type="submit" class="text-[11px] font-bold text-rausch underline">ارسال به بررسی</button>
                                    ...
                                    @elseif ($course->status === 'published')
                                        ...
                                        <button type="submit" class="text-[11px] font-bold text-muted underline">بایگانی</button>
                                    @endif
                                </form>
                            </div>
                        </td>
                        <td class="p-3.5 text-left">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.courses.edit', $course) }}" class="px-3 py-1.5 rounded-full border border-ink/20 font-bold hover:bg-surface-soft">ویرایش</a>
                ...
                @empty
                    <tr><td colspan="7" class="p-8 text-center text-sm text-muted">دوره‌ای با این مشخصات یافت نشد.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
