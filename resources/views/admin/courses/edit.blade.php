...
<section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="form-panel">
        <div class="flex items-center justify-between pb-4 border-b border-hairline-soft">
            <div>
                <h2 class="text-2xl font-bold text-ink">{{ $course->exists ? 'ویرایش دوره آموزشی' : 'ایجاد دوره آموزشی جدید' }}</h2>
                <p class="text-xs text-muted mt-1">اطلاعات محتوایی، اساتید پزشکی، سطح علمی و وضعیت انتشار دوره</p>
            </div>
            <a href="{{ route('admin.courses.index') }}" class="text-xs font-bold text-rausch underline">بازگشت به لیست دوره‌ها</a>
        </div>
        ...
        <form method="post" action="{{ $course->exists ? route('admin.courses.update', $course) : route('admin.courses.store') }}" class="mt-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    <label for="title" class="block text-xs font-bold text-ink">عنوان دوره (فارسی)</label>
                ...
                <div>
                    <label for="subject_id" class="block text-xs font-bold text-ink">مبحث / درس‌نامه</label>
                    <select id="subject_id" name="subject_id" required class="w-full mt-1.5 p-3 rounded-xl border border-ink/20 text-xs font-bold bg-white">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="reviewer_id" class="block text-xs font-bold text-ink">بازبین علمی / متخصص ناظر (باید متفاوت از نویسنده باشد)</label>
            ...
            <div>
                <label for="excerpt" class="block text-xs font-bold text-ink">چکیده کوتاه (برای کارت‌ها و پیش‌نمایش کاتالوگ)</label>
            ...
            <div>
                <label for="description" class="block text-xs font-bold text-ink">شرح و سرفصل‌های دوره</label>
                <textarea id="description" name="description" rows="5"
                          placeholder="اهداف آموزشی، مباحث بالینی و سرفصل‌های تدریس..."
                          class="w-full mt-1.5 p-3 rounded-xl border border-ink/20 text-xs font-medium bg-white">{{ old('description', $course->description) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label for="level" class="block text-xs font-bold text-ink">سطح علمی</label>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-hairline-soft">
                ...
                <a href="{{ route('admin.courses.index') }}" class="px-5 py-3 rounded-full border border-ink/20 text-xs font-bold text-muted hover:bg-surface-soft">
                    انصراف
                </a>
            </div>
        </form>
